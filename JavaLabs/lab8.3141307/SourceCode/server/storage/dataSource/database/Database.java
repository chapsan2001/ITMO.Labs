...
public final class Database extends DataSource {
  ...
  /**
   * Initializes usets table. Creates new one if not exists.
   *
   * @throws DatabaseException if initialization is incorrect
   */
  private void initUsersTable() throws DatabaseException {
    ...
    try {
      ...
      throw new DatabaseException("Error creating users table.", e);
    } finally {
  ...
  /**
   * Initializes music bands table. Creates new one if not exists.
   *
   * @throws DatabaseException if initialization is incorrect
   */
  private void initMusicBandsTable() throws DatabaseException {
    PreparedStatement preparedStatement =
        getPrepareStatement(CREATE_IF_NOT_EXISTS_MUSIC_BANDS_TABLE);

    try {
      preparedStatement.executeUpdate();
    } catch (SQLException | DAOException e) {
      throw new DatabaseException("Error creating music bands table.", e);
    } finally {
      closePrepareStatement(preparedStatement);
    }
  }
